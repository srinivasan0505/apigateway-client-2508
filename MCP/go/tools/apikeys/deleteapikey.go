...
func DeleteapikeyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeleteapikeyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_apikeys_api_Key",
		mcp.WithDescription("Deletes the ApiKey resource."),
		...
		mcp.WithString("api_Key", mcp.Required(), mcp.Description("The identifier of the ApiKey resource to be deleted.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeleteapikeyHandler(cfg),
	}
}
