...
func CreateapikeyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.CreateApiKeyrequest
		...
		url := fmt.Sprintf("%s/apikeys", cfg.BaseURL)
...
func CreateCreateapikeyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_apikeys",
		mcp.WithDescription("Create an ApiKey resource. "),
		...
		mcp.WithString("description", mcp.Description("Input parameter: The description of the ApiKey.")),
		mcp.WithBoolean("enabled", mcp.Description("Input parameter: Specifies whether the ApiKey can be used by callers.")),
		mcp.WithBoolean("generateDistinctId", mcp.Description("Input parameter: Specifies whether (<code>true</code>) or not (<code>false</code>) the key identifier is distinct from the created API key value. This parameter is deprecated and should not be used.")),
		mcp.WithString("name", mcp.Description("Input parameter: The name of the ApiKey.")),
		mcp.WithArray("stageKeys", mcp.Description("Input parameter: DEPRECATED FOR USAGE PLANS - Specifies stages associated with the API key.")),
		...
		mcp.WithString("value", mcp.Description("Input parameter: Specifies a value of the API key.")),
		mcp.WithString("customerId", mcp.Description("Input parameter: An AWS Marketplace customer identifier , when integrating with the AWS SaaS Marketplace.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    CreateapikeyHandler(cfg),
	}
}
