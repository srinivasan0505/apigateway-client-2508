...
func UpdateusageHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/usageplans/%s/keys/%s/usage", cfg.BaseURL, usageplanId, keyId)
...
func CreateUpdateusageTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_usageplans_usageplanId_keys_keyId_usage",
		mcp.WithDescription("Grants a temporary extension to the remaining quota of a usage plan associated with a specified API key."),
		...
		mcp.WithString("usageplanId", mcp.Required(), mcp.Description("The Id of the usage plan associated with the usage data.")),
		mcp.WithString("keyId", mcp.Required(), mcp.Description("The identifier of the API key associated with the usage plan in which a temporary extension is granted to the remaining quota.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    UpdateusageHandler(cfg),
	}
}
