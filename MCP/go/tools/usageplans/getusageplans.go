...
func GetusageplansHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/usageplans%s", cfg.BaseURL, queryString)
...
func CreateGetusageplansTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_usageplans",
		mcp.WithDescription("Gets all the usage plans of the caller's account."),
		...
		mcp.WithString("keyId", mcp.Description("The identifier of the API key associated with the usage plans.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetusageplansHandler(cfg),
	}
}
