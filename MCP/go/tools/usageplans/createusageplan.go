...
func CreateusageplanHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.CreateUsagePlanrequest
		...
		url := fmt.Sprintf("%s/usageplans", cfg.BaseURL)
...
func CreateCreateusageplanTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_usageplans",
		mcp.WithDescription("Creates a usage plan with the throttle and quota limits, as well as the associated API stages, specified in the payload. "),
		...
		mcp.WithObject("throttle", mcp.Description("Input parameter:  The API request rate limits.")),
		mcp.WithArray("apiStages", mcp.Description("Input parameter: The associated API stages of the usage plan.")),
		mcp.WithString("description", mcp.Description("Input parameter: The description of the usage plan.")),
		mcp.WithString("name", mcp.Required(), mcp.Description("Input parameter: The name of the usage plan.")),
		mcp.WithObject("quota", mcp.Description("Input parameter: Quotas configured for a usage plan.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    CreateusageplanHandler(cfg),
	}
}
