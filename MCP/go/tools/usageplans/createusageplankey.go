...
func CreateusageplankeyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.CreateUsagePlanKeyrequest
		...
		url := fmt.Sprintf("%s/usageplans/%s/keys", cfg.BaseURL, usageplanId)
...
func CreateCreateusageplankeyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_usageplans_usageplanId_keys",
		mcp.WithDescription("Creates a usage plan key for adding an existing API key to a usage plan."),
		...
		mcp.WithString("usageplanId", mcp.Required(), mcp.Description("The Id of the UsagePlan resource representing the usage plan containing the to-be-created UsagePlanKey resource representing a plan customer.")),
		mcp.WithString("keyType", mcp.Required(), mcp.Description("Input parameter: The type of a UsagePlanKey resource for a plan customer.")),
		mcp.WithString("keyId", mcp.Required(), mcp.Description("Input parameter: The identifier of a UsagePlanKey resource for a plan customer.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    CreateusageplankeyHandler(cfg),
	}
}
