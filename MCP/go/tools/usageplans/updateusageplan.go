...
func UpdateusageplanHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateUpdateusageplanTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_usageplans_usageplanId",
		mcp.WithDescription("Updates a usage plan of a given plan Id."),
		...
		mcp.WithString("usageplanId", mcp.Required(), mcp.Description("The Id of the to-be-updated usage plan.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    UpdateusageplanHandler(cfg),
	}
}
