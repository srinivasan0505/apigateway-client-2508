...
func DeleteusageplanHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeleteusageplanTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_usageplans_usageplanId",
		mcp.WithDescription("Deletes a usage plan of a given plan Id."),
		...
		mcp.WithString("usageplanId", mcp.Required(), mcp.Description("The Id of the to-be-deleted usage plan.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeleteusageplanHandler(cfg),
	}
}
