...
func GetsdktypeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		args, ok := request.Params.Arguments.(map[string]any)
		if !ok {
			return mcp.NewToolResultError("Invalid arguments object"), nil
		}
		sdktype_idVal, ok := args["sdktype_id"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: sdktype_id"), nil
		}
		sdktype_id, ok := sdktype_idVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: sdktype_id"), nil
		}
		url := fmt.Sprintf("%s/sdktypes/%s", cfg.BaseURL, sdktype_id)
...
func CreateGetsdktypeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_sdktypes_sdktype_id",
		mcp.WithDescription("Gets an SDK type."),
		...
		mcp.WithString("sdktype_id", mcp.Required(), mcp.Description("The identifier of the queried SdkType instance.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetsdktypeHandler(cfg),
	}
}
