...
func GetsdktypesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/sdktypes%s", cfg.BaseURL, queryString)
...
func CreateGetsdktypesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_sdktypes",
		mcp.WithDescription("Gets SDK types"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetsdktypesHandler(cfg),
	}
}
