...
func DeletevpclinkHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeletevpclinkTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_vpclinks_vpclink_id",
		mcp.WithDescription("Deletes an existing VpcLink of a specified identifier."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    DeletevpclinkHandler(cfg),
	}
}
