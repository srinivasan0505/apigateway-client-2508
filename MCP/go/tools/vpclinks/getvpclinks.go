...
func GetvpclinksHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/vpclinks%s", cfg.BaseURL, queryString)
...
func CreateGetvpclinksTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_vpclinks",
		mcp.WithDescription("Gets the VpcLinks collection under the caller's account in a selected region."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetvpclinksHandler(cfg),
	}
}
