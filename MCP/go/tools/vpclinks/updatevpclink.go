...
func UpdatevpclinkHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		args, ok := request.Params.Arguments.(map[string]any)
		if !ok {
			return mcp.NewToolResultError("Invalid arguments object"), nil
		}
		vpclink_idVal, ok := args["vpclink_id"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: vpclink_id"), nil
		}
		vpclink_id, ok := vpclink_idVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: vpclink_id"), nil
		}
		...
		url := fmt.Sprintf("%s/vpclinks/%s", cfg.BaseURL, vpclink_id)
...
func CreateUpdatevpclinkTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_vpclinks_vpclink_id",
		mcp.WithDescription("Updates an existing VpcLink of a specified identifier."),
		...
		mcp.WithString("vpclink_id", mcp.Required(), mcp.Description("The identifier of the VpcLink. It is used in an Integration to reference this VpcLink.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    UpdatevpclinkHandler(cfg),
	}
}
