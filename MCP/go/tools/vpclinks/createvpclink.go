...
func CreatevpclinkHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.CreateVpcLinkrequest
		...
		url := fmt.Sprintf("%s/vpclinks", cfg.BaseURL)
...
func CreateCreatevpclinkTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_vpclinks",
		mcp.WithDescription("Creates a VPC link, under the caller's account in a selected region, in an asynchronous operation that typically takes 2-4 minutes to complete and become operational. The caller must have permissions to create and update VPC Endpoint services."),
		...
		mcp.WithString("description", mcp.Description("Input parameter: The description of the VPC link.")),
		mcp.WithString("name", mcp.Required(), mcp.Description("Input parameter: The name used to label and identify the VPC link.")),
		...
		mcp.WithArray("targetArns", mcp.Required(), mcp.Description("Input parameter: The ARN of the network load balancer of the VPC targeted by the VPC link. The network load balancer must be owned by the same AWS account of the API owner.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    CreatevpclinkHandler(cfg),
	}
}
