...
func GetmodeltemplateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/restapis/%s/models/%s/default_template", cfg.BaseURL, restapi_id, model_name)
...
func CreateGetmodeltemplateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_restapis_restapi_id_models_model_name_default_template",
		mcp.WithDescription("Generates a sample mapping template that can be used to transform a payload into the structure of a model."),
		...
		mcp.WithString("model_name", mcp.Required(), mcp.Description("The name of the model for which to generate a template.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetmodeltemplateHandler(cfg),
	}
}
