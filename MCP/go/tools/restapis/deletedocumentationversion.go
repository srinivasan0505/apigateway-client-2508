...
func DeletedocumentationversionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeletedocumentationversionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_restapis_restapi_id_documentation_versions_doc_version",
		mcp.WithDescription("Deletes a documentation version."),
		...
		mcp.WithString("doc_version", mcp.Required(), mcp.Description("The version identifier of a to-be-deleted documentation snapshot.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeletedocumentationversionHandler(cfg),
	}
}
