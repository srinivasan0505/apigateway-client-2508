...
func UpdatedocumentationpartHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateUpdatedocumentationpartTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_restapis_restapi_id_documentation_parts_part_id",
		mcp.WithDescription("Updates a documentation part."),
		...
		mcp.WithString("part_id", mcp.Required(), mcp.Description("The identifier of the to-be-updated documentation part.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    UpdatedocumentationpartHandler(cfg),
	}
}
