...
func GetrequestvalidatorHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGetrequestvalidatorTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_restapis_restapi_id_requestvalidators_requestvalidator_id",
		mcp.WithDescription("Gets a RequestValidator of a given RestApi."),
		...
		mcp.WithString("requestvalidator_id", mcp.Required(), mcp.Description("The identifier of the RequestValidator to be retrieved.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetrequestvalidatorHandler(cfg),
	}
}
