...
func DeletestageHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeletestageTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_restapis_restapi_id_stages_stage_name",
		mcp.WithDescription("Deletes a Stage resource."),
		...
		mcp.WithString("stage_name", mcp.Required(), mcp.Description("The name of the Stage resource to delete.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeletestageHandler(cfg),
	}
}
