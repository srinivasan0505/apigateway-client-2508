...
func GetmodelHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		model_nameVal, ok := args["model_name"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: model_name"), nil
		}
		model_name, ok := model_nameVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: model_name"), nil
		}
		queryParams := make([]string, 0)
		if val, ok := args["flatten"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("flatten=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/restapis/%s/models/%s%s", cfg.BaseURL, restapi_id, model_name, queryString)
...
func CreateGetmodelTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_restapis_restapi_id_models_model_name",
		mcp.WithDescription("Describes an existing model defined for a RestApi resource."),
		...
		mcp.WithString("restapi_id", mcp.Required(), mcp.Description("The RestApi identifier under which the Model exists.")),
		mcp.WithString("model_name", mcp.Required(), mcp.Description("The name of the model as an identifier.")),
		mcp.WithBoolean("flatten", mcp.Description("A query parameter of a Boolean value to resolve (<code>true</code>) all external model references and returns a flattened model schema or not (<code>false</code>) The default is <code>false</code>.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetmodelHandler(cfg),
	}
}
