...
func CreatemodelHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.CreateModelrequest
		...
		url := fmt.Sprintf("%s/restapis/%s/models", cfg.BaseURL, restapi_id)
...
func CreateCreatemodelTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_restapis_restapi_id_models",
		mcp.WithDescription("Adds a new Model resource to an existing RestApi resource."),
		...
		mcp.WithString("restapi_id", mcp.Required(), mcp.Description("The RestApi identifier under which the Model will be created.")),
		mcp.WithString("name", mcp.Required(), mcp.Description("Input parameter: The name of the model. Must be alphanumeric.")),
		mcp.WithString("schema", mcp.Description("Input parameter: The schema for the model. For <code>application/json</code> models, this should be JSON schema draft 4 model.")),
		mcp.WithString("contentType", mcp.Required(), mcp.Description("Input parameter: The content-type for the model.")),
		mcp.WithString("description", mcp.Description("Input parameter: The description of the model.")),
	)
