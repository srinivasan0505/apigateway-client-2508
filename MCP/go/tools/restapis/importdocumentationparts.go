...
func ImportdocumentationpartsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.ImportDocumentationPartsrequest
		...
		var result models.DocumentationPartIds
...
func CreateImportdocumentationpartsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_restapis_restapi_id_documentation_parts",
		mcp.WithDescription("Imports documentation parts"),
		...
		mcp.WithString("mode", mcp.Description("A query parameter to indicate whether to overwrite (<code>OVERWRITE</code>) any existing DocumentationParts definition or to merge (<code>MERGE</code>) the new definition into the existing one. The default value is <code>MERGE</code>.")),
		mcp.WithBoolean("failonwarnings", mcp.Description("A query parameter to specify whether to rollback the documentation importation (<code>true</code>) or not (<code>false</code>) when a warning is encountered. The default value is <code>false</code>.")),
		mcp.WithString("body", mcp.Required(), mcp.Description("Input parameter: Raw byte array representing the to-be-imported documentation parts. To import from an OpenAPI file, this is a JSON object.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ImportdocumentationpartsHandler(cfg),
	}
}
