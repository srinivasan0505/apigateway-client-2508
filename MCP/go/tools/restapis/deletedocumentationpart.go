...
func DeletedocumentationpartHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeletedocumentationpartTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_restapis_restapi_id_documentation_parts_part_id",
		mcp.WithDescription("Deletes a documentation part"),
		...
		mcp.WithString("part_id", mcp.Required(), mcp.Description("The identifier of the to-be-deleted documentation part.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeletedocumentationpartHandler(cfg),
	}
}
