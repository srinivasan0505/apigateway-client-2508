...
func CreateresourceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		parent_idVal, ok := args["parent_id"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: parent_id"), nil
		}
		parent_id, ok := parent_idVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: parent_id"), nil
		}
		// Create properly typed request body using the generated schema
		var requestBody models.CreateResourcerequest
		...
		url := fmt.Sprintf("%s/restapis/%s/resources/%s", cfg.BaseURL, restapi_id, parent_id)
...
func CreateCreateresourceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_restapis_restapi_id_resources_parent_id",
		mcp.WithDescription("Creates a Resource resource."),
		...
		mcp.WithString("parent_id", mcp.Required(), mcp.Description("The parent resource's identifier.")),
		mcp.WithString("pathPart", mcp.Required(), mcp.Description("Input parameter: The last path segment for this resource.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    CreateresourceHandler(cfg),
	}
}
