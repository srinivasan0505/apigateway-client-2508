...
func UpdatestageHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		stage_nameVal, ok := args["stage_name"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: stage_name"), nil
		}
		stage_name, ok := stage_nameVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: stage_name"), nil
		}
		...
		url := fmt.Sprintf("%s/restapis/%s/stages/%s", cfg.BaseURL, restapi_id, stage_name)
...
func CreateUpdatestageTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_restapis_restapi_id_stages_stage_name",
		mcp.WithDescription("Changes information about a Stage resource."),
		...
		mcp.WithString("stage_name", mcp.Required(), mcp.Description("The name of the Stage resource to change information about.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    UpdatestageHandler(cfg),
	}
}
