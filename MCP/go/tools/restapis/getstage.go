...
func GetstageHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGetstageTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_restapis_restapi_id_stages_stage_name",
		mcp.WithDescription("Gets information about a Stage resource."),
		...
		mcp.WithString("stage_name", mcp.Required(), mcp.Description("The name of the Stage resource to get information about.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetstageHandler(cfg),
	}
}
