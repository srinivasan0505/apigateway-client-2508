...
func GetmodelsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/restapis/%s/models%s", cfg.BaseURL, restapi_id, queryString)
...
func CreateGetmodelsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_restapis_restapi_id_models",
		mcp.WithDescription("Describes existing Models defined for a RestApi resource."),
