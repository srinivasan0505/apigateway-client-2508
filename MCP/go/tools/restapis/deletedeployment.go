...
func DeletedeploymentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeletedeploymentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_restapis_restapi_id_deployments_deployment_id",
		mcp.WithDescription("Deletes a Deployment resource. Deleting a deployment will only succeed if there are no Stage resources associated with it."),
		...
		mcp.WithString("deployment_id", mcp.Required(), mcp.Description("The identifier of the Deployment resource to delete.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeletedeploymentHandler(cfg),
	}
}
