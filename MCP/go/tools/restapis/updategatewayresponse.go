...
func UpdategatewayresponseHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateUpdategatewayresponseTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_restapis_restapi_id_gatewayresponses_response_type",
		mcp.WithDescription("Updates a GatewayResponse of a specified response type on the given RestApi."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    UpdategatewayresponseHandler(cfg),
	}
}
