...
func CreatedocumentationpartHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.CreateDocumentationPartrequest
		...
		url := fmt.Sprintf("%s/restapis/%s/documentation/parts", cfg.BaseURL, restapi_id)
...
func CreateCreatedocumentationpartTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_restapis_restapi_id_documentation_parts",
		mcp.WithDescription("Creates a documentation part."),
		...
		mcp.WithObject("location", mcp.Required(), mcp.Description("Input parameter: Specifies the target API entity to which the documentation applies.")),
		mcp.WithString("properties", mcp.Required(), mcp.Description("Input parameter: The new documentation content map of the targeted API entity. Enclosed key-value pairs are API-specific, but only OpenAPI-compliant key-value pairs can be exported and, hence, published.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    CreatedocumentationpartHandler(cfg),
	}
}
