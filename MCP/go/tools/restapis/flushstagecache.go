...
func FlushstagecacheHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/restapis/%s/stages/%s/cache/data", cfg.BaseURL, restapi_id, stage_name)
...
func CreateFlushstagecacheTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_restapis_restapi_id_stages_stage_name_cache_data",
		mcp.WithDescription("Flushes a stage's cache."),
		...
		mcp.WithString("stage_name", mcp.Required(), mcp.Description("The name of the stage to flush its cache.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    FlushstagecacheHandler(cfg),
	}
}
