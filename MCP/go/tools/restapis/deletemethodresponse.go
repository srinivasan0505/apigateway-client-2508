...
func DeletemethodresponseHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeletemethodresponseTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_restapis_restapi_id_resources_resource_id_methods_http_method_responses_status_code",
		mcp.WithDescription("Deletes an existing MethodResponse resource."),
		...
		mcp.WithString("status_code", mcp.Required(), mcp.Description("The status code identifier for the MethodResponse resource.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeletemethodresponseHandler(cfg),
	}
}
