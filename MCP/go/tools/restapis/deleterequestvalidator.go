...
func DeleterequestvalidatorHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeleterequestvalidatorTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_restapis_restapi_id_requestvalidators_requestvalidator_id",
		mcp.WithDescription("Deletes a RequestValidator of a given RestApi."),
		...
		mcp.WithString("requestvalidator_id", mcp.Required(), mcp.Description("The identifier of the RequestValidator to be deleted.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeleterequestvalidatorHandler(cfg),
	}
}
