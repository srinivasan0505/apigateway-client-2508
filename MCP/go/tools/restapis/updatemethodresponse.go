...
func UpdatemethodresponseHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateUpdatemethodresponseTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_restapis_restapi_id_resources_resource_id_methods_http_method_responses_status_code",
		mcp.WithDescription("Updates an existing MethodResponse resource."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    UpdatemethodresponseHandler(cfg),
	}
}
