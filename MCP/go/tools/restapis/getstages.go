...
func GetstagesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["deploymentId"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("deploymentId=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/restapis/%s/stages%s", cfg.BaseURL, restapi_id, queryString)
...
func CreateGetstagesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_restapis_restapi_id_stages",
		mcp.WithDescription("Gets information about one or more Stage resources."),
		...
		mcp.WithString("deploymentId", mcp.Description("The stages' deployment identifiers.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetstagesHandler(cfg),
	}
}
