...
func DeleteintegrationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeleteintegrationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_restapis_restapi_id_resources_resource_id_methods_http_method_integration",
		mcp.WithDescription("Represents a delete integration."),
		...
		mcp.WithString("resource_id", mcp.Required(), mcp.Description("Specifies a delete integration request's resource identifier.")),
		mcp.WithString("http_method", mcp.Required(), mcp.Description("Specifies a delete integration request's HTTP method.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeleteintegrationHandler(cfg),
	}
}
