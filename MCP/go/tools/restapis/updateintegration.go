...
func UpdateintegrationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/restapis/%s/resources/%s/methods/%s/integration", cfg.BaseURL, restapi_id, resource_id, http_method)
...
func CreateUpdateintegrationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_restapis_restapi_id_resources_resource_id_methods_http_method_integration",
		mcp.WithDescription("Represents an update integration."),
		...
		mcp.WithString("resource_id", mcp.Required(), mcp.Description("Represents an update integration request's resource identifier.")),
		mcp.WithString("http_method", mcp.Required(), mcp.Description("Represents an update integration request's HTTP method.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    UpdateintegrationHandler(cfg),
	}
}
