...
func CreaterequestvalidatorHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.CreateRequestValidatorrequest
		...
		url := fmt.Sprintf("%s/restapis/%s/requestvalidators", cfg.BaseURL, restapi_id)
...
func CreateCreaterequestvalidatorTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_restapis_restapi_id_requestvalidators",
		mcp.WithDescription("Creates a RequestValidator of a given RestApi."),
		...
		mcp.WithString("name", mcp.Description("Input parameter: The name of the to-be-created RequestValidator.")),
		mcp.WithBoolean("validateRequestBody", mcp.Description("Input parameter: A Boolean flag to indicate whether to validate request body according to the configured model schema for the method (<code>true</code>) or not (<code>false</code>).")),
		mcp.WithBoolean("validateRequestParameters", mcp.Description("Input parameter: A Boolean flag to indicate whether to validate request parameters, <code>true</code>, or not <code>false</code>.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    CreaterequestvalidatorHandler(cfg),
	}
}
