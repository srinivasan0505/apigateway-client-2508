...
func UpdaterequestvalidatorHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		requestvalidator_idVal, ok := args["requestvalidator_id"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: requestvalidator_id"), nil
		}
		requestvalidator_id, ok := requestvalidator_idVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: requestvalidator_id"), nil
		}
		// Create properly typed request body using the generated schema
		var requestBody models.UpdateApiKeyrequest
		...
		url := fmt.Sprintf("%s/restapis/%s/requestvalidators/%s", cfg.BaseURL, restapi_id, requestvalidator_id)
		...
		var result models.RequestValidator
...
func CreateUpdaterequestvalidatorTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_restapis_restapi_id_requestvalidators_requestvalidator_id",
		mcp.WithDescription("Updates a RequestValidator of a given RestApi."),
		...
		mcp.WithString("requestvalidator_id", mcp.Required(), mcp.Description("The identifier of RequestValidator to be updated.")),
		mcp.WithArray("patchOperations", mcp.Description("Input parameter: A list of operations describing the updates to apply to the specified resource. The patches are applied in the order specified in the list.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    UpdaterequestvalidatorHandler(cfg),
	}
}
