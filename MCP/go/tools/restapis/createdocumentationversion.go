...
func CreatedocumentationversionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.CreateDocumentationVersionrequest
		...
		url := fmt.Sprintf("%s/restapis/%s/documentation/versions", cfg.BaseURL, restapi_id)
...
func CreateCreatedocumentationversionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_restapis_restapi_id_documentation_versions",
		mcp.WithDescription("Creates a documentation version"),
		...
		mcp.WithString("description", mcp.Description("Input parameter: A description about the new documentation snapshot.")),
		mcp.WithString("documentationVersion", mcp.Required(), mcp.Description("Input parameter: The version identifier of the new snapshot.")),
		mcp.WithString("stageName", mcp.Description("Input parameter: The stage name to be associated with the new documentation snapshot.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    CreatedocumentationversionHandler(cfg),
	}
}
