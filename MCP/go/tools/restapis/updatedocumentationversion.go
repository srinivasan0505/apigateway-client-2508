...
func UpdatedocumentationversionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		doc_versionVal, ok := args["doc_version"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: doc_version"), nil
		}
		doc_version, ok := doc_versionVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: doc_version"), nil
		}
		...
		url := fmt.Sprintf("%s/restapis/%s/documentation/versions/%s", cfg.BaseURL, restapi_id, doc_version)
		...
		var result models.DocumentationVersion
...
func CreateUpdatedocumentationversionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_restapis_restapi_id_documentation_versions_doc_version",
		mcp.WithDescription("Updates a documentation version."),
		...
		mcp.WithString("doc_version", mcp.Required(), mcp.Description("The version identifier of the to-be-updated documentation version.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    UpdatedocumentationversionHandler(cfg),
	}
}
