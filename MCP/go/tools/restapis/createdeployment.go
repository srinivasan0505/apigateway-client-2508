...
func CreatedeploymentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/restapis/%s/deployments", cfg.BaseURL, restapi_id)
...
func CreateCreatedeploymentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_restapis_restapi_id_deployments",
		mcp.WithDescription("Creates a Deployment resource, which makes a specified RestApi callable over the internet."),
		...
		mcp.WithString("stageDescription", mcp.Description("Input parameter: The description of the Stage resource for the Deployment resource to create.")),
		mcp.WithString("stageName", mcp.Description("Input parameter: The name of the Stage resource for the Deployment resource to create.")),
		...
		mcp.WithObject("variables", mcp.Description("Input parameter: A map that defines the stage variables for the Stage resource that is associated with the new deployment. Variable names can have alphanumeric and underscore characters, and the values must match <code>[A-Za-z0-9-._~:/?#&amp;=,]+</code>.")),
		mcp.WithBoolean("cacheClusterEnabled", mcp.Description("Input parameter: Enables a cache cluster for the Stage resource specified in the input.")),
		mcp.WithObject("canarySettings", mcp.Description("Input parameter: The input configuration for a canary deployment.")),
		mcp.WithString("description", mcp.Description("Input parameter: The description for the Deployment resource to create.")),
	)
