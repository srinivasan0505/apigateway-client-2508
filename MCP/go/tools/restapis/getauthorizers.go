...
func GetauthorizersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/restapis/%s/authorizers%s", cfg.BaseURL, restapi_id, queryString)
...
func CreateGetauthorizersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_restapis_restapi_id_authorizers",
		mcp.WithDescription("Describe an existing Authorizers resource."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetauthorizersHandler(cfg),
	}
}
