...
func GetgatewayresponsesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/restapis/%s/gatewayresponses%s", cfg.BaseURL, restapi_id, queryString)
		...
		var result models.GatewayResponses
...
func CreateGetgatewayresponsesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_restapis_restapi_id_gatewayresponses",
		mcp.WithDescription("Gets the GatewayResponses collection on the given RestApi. If an API developer has not added any definitions for gateway responses, the result will be the API Gateway-generated default GatewayResponses collection for the supported response types."),
		...
		mcp.WithString("position", mcp.Description("The current pagination position in the paged result set. The GatewayResponse collection does not support pagination and the position does not apply here.")),
		mcp.WithNumber("limit", mcp.Description("The maximum number of returned results per page. The default value is 25 and the maximum value is 500. The GatewayResponses collection does not support pagination and the limit does not apply here.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetgatewayresponsesHandler(cfg),
	}
}
