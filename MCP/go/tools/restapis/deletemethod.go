...
func DeletemethodHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeletemethodTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_restapis_restapi_id_resources_resource_id_methods_http_method",
		mcp.WithDescription("Deletes an existing Method resource."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    DeletemethodHandler(cfg),
	}
}
