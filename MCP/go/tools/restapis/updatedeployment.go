...
func UpdatedeploymentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/restapis/%s/deployments/%s", cfg.BaseURL, restapi_id, deployment_id)
...
func CreateUpdatedeploymentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_restapis_restapi_id_deployments_deployment_id",
		mcp.WithDescription("Changes information about a Deployment resource."),
		...
		mcp.WithString("deployment_id", mcp.Required(), mcp.Description("The replacement identifier for the Deployment resource to change information about.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    UpdatedeploymentHandler(cfg),
	}
}
