...
func DeleteauthorizerHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeleteauthorizerTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_restapis_restapi_id_authorizers_authorizer_id",
		mcp.WithDescription("Deletes an existing Authorizer resource."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    DeleteauthorizerHandler(cfg),
	}
}
