...
func GetintegrationresponseHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGetintegrationresponseTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_restapis_restapi_id_resources_resource_id_methods_http_method_integration_responses_status_code",
		mcp.WithDescription("Represents a get integration response."),
		...
		mcp.WithString("resource_id", mcp.Required(), mcp.Description("Specifies a get integration response request's resource identifier.")),
		mcp.WithString("http_method", mcp.Required(), mcp.Description("Specifies a get integration response request's HTTP method.")),
		mcp.WithString("status_code", mcp.Required(), mcp.Description("Specifies a get integration response request's status code.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetintegrationresponseHandler(cfg),
	}
}
