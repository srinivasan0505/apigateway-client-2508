...
func UpdateauthorizerHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateUpdateauthorizerTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_restapis_restapi_id_authorizers_authorizer_id",
		mcp.WithDescription("Updates an existing Authorizer resource."),
		...
		mcp.WithString("authorizer_id", mcp.Required(), mcp.Description("The identifier of the Authorizer resource.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    UpdateauthorizerHandler(cfg),
	}
}
