...
func GetintegrationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGetintegrationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_restapis_restapi_id_resources_resource_id_methods_http_method_integration",
		...
		mcp.WithString("resource_id", mcp.Required(), mcp.Description("Specifies a get integration request's resource identifier")),
		mcp.WithString("http_method", mcp.Required(), mcp.Description("Specifies a get integration request's HTTP method.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetintegrationHandler(cfg),
	}
}
