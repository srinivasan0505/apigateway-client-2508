...
func DeleterestapiHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeleterestapiTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_restapis_restapi_id",
		mcp.WithDescription("Deletes the specified API."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    DeleterestapiHandler(cfg),
	}
}
