...
func GetgatewayresponseHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGetgatewayresponseTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_restapis_restapi_id_gatewayresponses_response_type",
		mcp.WithDescription("Gets a GatewayResponse of a specified response type on the given RestApi."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetgatewayresponseHandler(cfg),
	}
}
