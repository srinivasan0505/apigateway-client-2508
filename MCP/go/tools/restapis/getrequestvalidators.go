...
func GetrequestvalidatorsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/restapis/%s/requestvalidators%s", cfg.BaseURL, restapi_id, queryString)
		...
		var result models.RequestValidators
...
func CreateGetrequestvalidatorsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_restapis_restapi_id_requestvalidators",
		mcp.WithDescription("Gets the RequestValidators collection of a given RestApi."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetrequestvalidatorsHandler(cfg),
	}
}
