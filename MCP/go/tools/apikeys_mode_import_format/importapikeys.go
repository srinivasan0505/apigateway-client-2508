...
func ImportapikeysHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.ImportApiKeysrequest
		...
		url := fmt.Sprintf("%s/apikeys#mode=import&format%s", cfg.BaseURL, queryString)
...
func CreateImportapikeysTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_apikeys#mode=import&format",
		mcp.WithDescription("Import API keys from an external source, such as a CSV-formatted file."),
		...
		mcp.WithString("format", mcp.Required(), mcp.Description("A query parameter to specify the input format to imported API keys. Currently, only the <code>csv</code> format is supported.")),
		mcp.WithBoolean("failonwarnings", mcp.Description("A query parameter to indicate whether to rollback ApiKey importation (<code>true</code>) or not (<code>false</code>) when error is encountered.")),
		mcp.WithString("mode", mcp.Required(), mcp.Description("")),
		mcp.WithString("body", mcp.Required(), mcp.Description("Input parameter: The payload of the POST request to import API keys. For the payload format, see API Key File Format.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ImportapikeysHandler(cfg),
	}
}
