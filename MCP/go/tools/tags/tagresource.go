...
func CreateTagresourceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_tags_resource_arn",
		mcp.WithDescription("Adds or updates a tag on a given resource."),
		...
		mcp.WithObject("tags", mcp.Required(), mcp.Description("Input parameter: The key-value map of strings. The valid character set is [a-zA-Z+-=._:/]. The tag key can be up to 128 characters and must not start with <code>aws:</code>. The tag value can be up to 256 characters.")),
	)
