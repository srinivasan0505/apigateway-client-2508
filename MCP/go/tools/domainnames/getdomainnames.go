...
func GetdomainnamesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/domainnames%s", cfg.BaseURL, queryString)
...
func CreateGetdomainnamesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_domainnames",
		mcp.WithDescription("Represents a collection of DomainName resources."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetdomainnamesHandler(cfg),
	}
}
