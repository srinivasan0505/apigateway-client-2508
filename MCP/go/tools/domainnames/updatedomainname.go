...
func UpdatedomainnameHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateUpdatedomainnameTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_domainnames_domain_name",
		mcp.WithDescription("Changes information about the DomainName resource."),
		...
		mcp.WithString("domain_name", mcp.Required(), mcp.Description("The name of the DomainName resource to be changed.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    UpdatedomainnameHandler(cfg),
	}
}
