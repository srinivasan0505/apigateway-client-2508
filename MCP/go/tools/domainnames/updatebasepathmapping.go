...
func UpdatebasepathmappingHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateUpdatebasepathmappingTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_domainnames_domain_name_basepathmappings_base_path",
		mcp.WithDescription("Changes information about the BasePathMapping resource."),
		...
		mcp.WithString("domain_name", mcp.Required(), mcp.Description("The domain name of the BasePathMapping resource to change.")),
		mcp.WithString("base_path", mcp.Required(), mcp.Description("<p>The base path of the BasePathMapping resource to change.</p> <p>To specify an empty base path, set this parameter to <code>'(none)'</code>.</p>")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    UpdatebasepathmappingHandler(cfg),
	}
}
