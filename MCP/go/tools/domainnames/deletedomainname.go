...
func DeletedomainnameHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeletedomainnameTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_domainnames_domain_name",
		mcp.WithDescription("Deletes the DomainName resource."),
		...
		mcp.WithString("domain_name", mcp.Required(), mcp.Description("The name of the DomainName resource to be deleted.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeletedomainnameHandler(cfg),
	}
}
