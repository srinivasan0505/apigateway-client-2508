...
func UpdateclientcertificateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateUpdateclientcertificateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_clientcertificates_clientcertificate_id",
		mcp.WithDescription("Changes information about an ClientCertificate resource."),
		...
		mcp.WithString("clientcertificate_id", mcp.Required(), mcp.Description("The identifier of the ClientCertificate resource to be updated.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    UpdateclientcertificateHandler(cfg),
	}
}
