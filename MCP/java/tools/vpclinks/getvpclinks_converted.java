/**
 * MCP Server function for Gets the VpcLinks collection under the caller's account in a selected region.
 */
...
class Get_VpclinksMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getGet_VpclinksHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("limit")) {
            queryParams.add("limit=" + args.get("limit"));
                ...
                String url = config.getBaseUrl() + "/api/v2/get_vpclinks" + queryString;
    ...
    public static MCPServer.Tool createGet_VpclinksTool(MCPServer.APIConfig config) {
        ...
        return new MCPServer.Tool(definition, getGet_VpclinksHandler(config));
    }
